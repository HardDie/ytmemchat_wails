...
* **Unit tests** live next to the package (`foo_test.go`). Run on every push: `go test -race -tags=nomain .` then `go test -race -tags=nomain ./bindings/...` then `go test -race ./internal/...`.
