...
* **Releases:** tag `v1.2.3` (semver with `v` prefix). Checkout with `fetch-depth: 0` so tags exist. Build with `wails build -platform … -ldflags "-X main.buildVersion=…"` (exact tag or short commit) on `ubuntu-latest`, `ubuntu-24.04-arm`, `windows-latest`, `macos-latest` (`darwin/universal`). Attach archives plus `SHA256SUMS.txt` to the GitHub Release.
