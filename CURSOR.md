...
6. **Bindings**: `GetSettings` / `SaveSettings`, `GetOBSStatus`, `Start` / `Stop` / `GetRunStatus`, `GetTTSVoices`, `SendTestMessage`, `InterruptTTS`, file/folder pickers. Start uses saved settings (window saves the form first). Empty API key → `nokey`; non-empty → v3 only. Never fall back on invalid key. Connect runs in a goroutine. After each chat line (YouTube, `POST /api/webhook`, or **Send** in the window): overlay `alert` on a command match, else TTS when enabled. Inject/test run while OBS HTTP is up; YouTube Start is not required. Empty `commandsFilePath` skips the matcher; a bad file fails Start (inject logs and skips the matcher).
7. **What belongs here**: stream ID (required to Start), optional YouTube API key, listen port, TTS on/off + voice, alerts on/off + command token + media/commands paths, webhook on/off, optional window size. The window is setup only (not on stream). **Home**: YouTube status, Start/Stop, interrupt overlay audio. **Config**: all settings and OBS URLs. **Test**: send a fake chat line. **What does not**: chat history, live iterator state.
