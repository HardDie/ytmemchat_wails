...
- `internal/` must stay **CGO-free** so CI can test on Ubuntu without WebKit. Do not import Wails from `internal/`. OS hotkey registration lives in package main (`!nomain`).
...
- **Import direction:** `app.go` → config, youtube, obs, alerts, tts, hotkey. `alerts` / `tts` may depend on overlay event types in `obs`. `obs` must not import `alerts` or `youtube` (register handlers from `app.go`).
